...
static int sderror(struct sdcore *sd, unsigned int *resp, unsigned short req) {
...
static int stop(struct sdcore *sd) {
	...
	if (sd->parked_sector) {
		...
		sd->parked_sector = 0;
		if (ret||sderror(sd, resp, CMD_STOP_TRANSMISSION)||timeout(sd)) 
		  return 1;
	}
	return 0;
}
...
static int do_read(struct sdcore *sd, unsigned int sector, struct sdiov *iov, 
  unsigned int iovcnt) {
	...
	if (ret || sderror(sd, resp, CMD_READ_MULTIPLE_BLOCK)) return 1;
  ...
  unsigned int iovcnt) {
	...
	if (sd->sd_erasehint) {
		...
		ret = sdcmd2(sd, ACMD_SET_WR_BLK_ERASE_COUNT, sd->sd_erasehint, 
		  resp, NULL);
		if (ret||sderror(sd,resp, ACMD_SET_WR_BLK_ERASE_COUNT)) return 1;
		sd->sd_erasehint = 0;
	}
	...
	if (sd->sd_nomultiwrite || sector == sz - 1) for (;;) {
		...
		if (ret || sderror(sd, resp, CMD_WRITE_BLOCK)) return 1;
	...
	if (ret || sderror(sd, resp, CMD_WRITE_MULTIPLE_BLOCK)) {
...
int sdreset(struct sdcore *sd) {
	...
	*/
	...
	if ((SDPEEK8(sd, SDCTRL) & 0x80) || (sd->sd_csd[15] & 0x30))
		sd->sd_wprot = 1;
	sd->sd_blocksize = 1 << ((sd->sd_csd[6] & 0xf));
	if (timeout(sd) || sderror(sd, resp, ACMD_SET_BUS_WIDTH)){
		sd->refcnt--;
		return 0;
	} else { 
		sd->refcnt--;
...
static int sdsetwprot2(struct sdcore *sd, unsigned int perm) {
	...
	if (ret || sderror(sd, resp, CMD_PROGRAM_CSD)) return 1;
	for (i = 0; i < 16; i++) {
		s = *csdptr++;
...
int sdlockctl(struct sdcore *sd, unsigned int cmd, unsigned char *pwd, 
  unsigned char *sdbootdat) {
	...
	if (ret || sderror(sd, resp, CMD_SET_BLOCKLEN)) return 1;
	ret = sdcmd(sd, CMD_LOCK_UNLOCK, 0, resp, NULL);
	if (ret || sderror(sd, resp, CMD_LOCK_UNLOCK)) return 1;
	...
	if (ret || sderror(sd, resp, CMD_SEND_STATUS)) return 1;
...
int sdlockctl2(struct sdcore *sd, unsigned int cmd, unsigned char *pwd, 
  unsigned char *sdbootdat) {
	...
	if (ret || sderror(sd, resp, CMD_SET_BLOCKLEN)) {
		return 1;
	}
	ret = sdcmd2(sd, CMD_SEND_STATUS, sd->sd_rcaarg, resp, NULL);
	if (ret || sderror(sd, resp, CMD_SEND_STATUS)) {
		return 1;
	}
